...
/**
 * Similar to fileno(3), but taking a C++ stream as argument instead of a FILE*.
 * Note that there is no way for the library to track what you do with the
 * descriptor, so be careful.
 *
 * @return  The integer file descriptor associated with the stream, or -1 if
 *          that stream is invalid.  In the latter case, for the sake of
 *          keeping the code as similar to fileno(3), errno is set to EBADF.
 *
 * @see     The <A HREF="http://www.ginac.de/~kreckel/fileno/">upstream page
 *          at http://www.ginac.de/~kreckel/fileno/</A> of this code provides
 *          more detailed information.
 */

template <typename charT, typename traits>
__attribute__((always_inline))
static inline int fileno_hack(const std::basic_ios<charT, traits>& stream)
{
...
// OK, stop reading here, because it's getting obscene.  Cross fingers!
    ...
    if (fbuf != NULL) {
        struct my_filebuf : public filebuf_t {
            ...
            __attribute__((always_inline)) inline FILE* c_file()
            {
                std::__basic_file<charT>* ptr_M_file = *(std::__basic_file<charT>**)((char*)this
                    + sizeof(std::basic_streambuf<charT, traits>));
